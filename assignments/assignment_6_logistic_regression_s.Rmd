...
c(is.factor(data$Survived), is.factor(data$Pclass), is.factor(data$Sex), is.factor(data$Embarked))
c(levels(data$Survived), levels(data$Pclass), levels(data$Sex), levels(data$Embarked))

# Transforming "Cabin" into factor
data <- data %>%
  mutate(Cabin_factor = factor(ifelse(Cabin == "", 0, 1), 
                               levels = c(0, 1), 
                               labels = c("No cabin", "Had cabin")))

is.factor(data$Cabin_factor)

# Cheking factors' counts with barplots
...
# Computing standard deviations for numeric variables
